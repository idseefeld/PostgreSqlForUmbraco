...
public class PostgreSqlSyntaxProvider : SqlSyntaxProviderBase<PostgreSqlSyntaxProvider>
{
        ...
        IPackagesService packagesService)
    {
        ...
        DateTimeColumnDefinition = "TIMESTAMPTZ"; // "TIMESTAMPTZ";     // "WITH TIME ZONE";
        DateTimeOffsetColumnDefinition = "TIMESTAMPTZ"; // "TIMESTAMP"; // "WITHOUT TIME ZONE";
        TimeColumnDefinition = "TIME";
        DecimalColumnDefinition = string.Format("NUMERIC({0},{1})", DefaultDecimalPrecision, DefaultDecimalScale);
    ...
    public override void HandleCreateTable(IDatabase database, TableDefinition tableDefinition, bool skipKeysAndIndexes = false)
    {
        ...
        var executeSql = new Sql(createSql);
        ...
        if (string.IsNullOrEmpty(createPrimaryKeySql) == false)
        {
            ...
            executeSql = new Sql(createPrimaryKeySql);
            database.Execute(executeSql);
        }
    ...
    public override string TruncateConstraintName<T>(string constraintName)
    {
        ...
        if (constraintName.Length <= PostgreSqlMaxIdentifierLength)
        {
            return constraintName;
        }
        ...
        string hash = GetStableHexHash(constraintName);
        string prefix = constraintName.Substring(0, maxPrefixLength);
    ...
    private string BuildForeignKeyConstraintName(ForeignKeyDefinition foreignKey)
    {
    ...
    /// <inheritdoc />
    private string CreateConstraintInline => "CONSTRAINT {1} {2} ({3})";
    ...
    protected override string FormatConstraint(ColumnDefinition column)
    {
        ...
        return $"CONSTRAINT {(string.IsNullOrEmpty(column.ConstraintName) ? GetQuotedName($"DF_{column.TableName}_{column.Name}") : column.ConstraintName)}";
    }
    ...
    protected override string? FormatSystemMethods(SystemMethods systemMethod)
    {
        ...
        // see: select now() as datetime_tz, timezone(now()) as local_time, timezone('utc'::text, now()) as utc
